...
class BeautifulPicture():
    ...
    def get_pic(self):
        ...
        print('开始获取所有a标签')
        all_a = BeautifulSoup(r.text, 'lxml').find_all('img',src="include/Code.asp")  #获取网页中的class为cV68d的所有a标签
        print(all_a)
        print('开始创建文件夹')
        ...
        for a in all_a: #循环每个标签，获取标签中图片的url并且进行网络请求，最后保存图片
            ...
            img_url_final = self.web_url + img_str
            print('img = ',img_url_final)
            #获取高度和宽度的字符在字符串中的位置
            ...
            img_name = 'code'#img_url[name_start_pos : name_end_pos]
